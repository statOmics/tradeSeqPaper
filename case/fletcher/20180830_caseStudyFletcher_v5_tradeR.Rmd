...
# gene set enrichment analysis with a custom background?

# + which ones do we find extra?
...
plotSmoothersIk <- function(m, nPoints = 100, ...) {
  ...
  legend("topright", c("Neuronal", "Microvillous", "Sustentacular"),
    col = cols,
...
for(ii in 1:length(earlyTFs)) plotSmoothersIk(gamList[[earlyTFs[ii]]], main=earlyTFs[ii])
dev.off()
...
for (ii in 1:9) {
  plotSmoothersIk(gamList[[sigAll13[ii]]], ylim = c(0, 10), main = sigAll13[ii])
}
dev.off()
