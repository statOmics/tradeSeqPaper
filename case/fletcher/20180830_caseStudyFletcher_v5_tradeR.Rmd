...
### cell cycle patterns within lineages: tradeR

```{r}
assocRes <- associationTest(gamList, omnibus=TRUE, lineages=TRUE)
deAssoc1 <- rownames(assocRes)[p.adjust(assocRes$pvalue_1,"fdr")<=0.05]
deAssoc1 <- deAssoc1[!is.na(deAssoc1)]
length(deAssoc1) #1931 DE genes
...
# relative
plot(x=1:nrow(assocRes), y=sumFoundCC/sumFoundRandom, type="l", ylab="# genes found with tradeR / # genes found under random selection")
abline(h=1, lty=2, col="red")

o1 <- order(assocRes[deAssoc1,"waldStat_1"], decreasing=TRUE)
top200Assoc <- deAssoc1[o1[1:200]]
#clusAssoc1 <- clusterExpressionPatterns(gamList, nPoints=20, genes=top200Assoc, ncores=2)

# heatmap for neuronal lineage.
source("/Users/koenvandenberge/Dropbox/PhD/Research/singleCell/trajectoryInference/trajectoryDE/tradeR/R/utils.R")
...
df$ct[df$t1>=80 & df$t1<115] <- "HBC*"
df$cols[df$t1>=80 & df$t1<115] <- colpal[7]
df$ct[df$t1>=115 & df$t1<220] <- "GBC"
df$cols[df$t1>=115 & df$t1<220] <- colpal[5]
df$ct[df$t1>=220 & df$t1<250] <- "INP1"
df$cols[df$t1>=220 & df$t1<250] <- colpal[2]
df$ct[df$t1>=250 & df$t1<310] <- "INP2"
df$cols[df$t1>=250 & df$t1<310] <- colpal[12]
df$ct[df$t1>=310 & df$t1<325] <- "INP3"
df$cols[df$t1>=310 & df$t1<325] <- colpal[9]
df$ct[df$t1>=325 & df$t1<385] <- "iOSN"
df$cols[df$t1>=325 & df$t1<385] <- colpal[8]
df$ct[df$t1>=385] <- "mOSN"
df$cols[df$t1>=385] <- colpal[11]
