...
```{r}
pdf("~/Dropbox/PhD/Research/singleCell/trajectoryInference/trajectoryDE/tradeRPaper/simulation/sim2_dyntoy/falseGenesSlingshot.pdf")
for(ii in 1:length(falseGenes)){
  print(plotSmoothers(gamList[[falseGenes[ii]]]))
}
dev.off()
```
...
# If we do not specify the root of the Monocle trajectory, it has the start of the lineage wrong, which obscures downstream analysis.
cds <- orderCells(cds, root_state=7)#, num_paths=2)
plot_cell_trajectory(cds, color_by = "State")
...
# state 6 and state 1 are the two lineages.
cellWeightsMon <- matrix(0, nrow=ncol(cds), ncol=2)
rownames(cellWeightsMon) <- colnames(counts)
cellWeightsMon[stateMon==1,1] <- 1
cellWeightsMon[stateMon==6,2] <- 1
cellWeightsMon[stateMon!=c(1,6),] <- c(0.5, 0.5)
...
# if not re-rooted, the end of slingshot lineage is the middle of Monocle lineage
#mypar(mfrow=c(1,2))
#i=27 ; plotSmoothers(gamListMon[[i]]) ; plotSmoothers(gamList[[i]])
...
"tradeR_Monocle2_end", "tradeR_Monocle2_pattern")
...
score <- data.frame(GPfates=GPfatesBif$D,
                      row.names=rownames(counts))
cobra <- COBRAData(pval=pval, truth=truth, score=score)
saveRDS(cobra,file="~/Dropbox/PhD/Research/singleCell/trajectoryInference/trajectoryDE/tradeRPaper/simulation/sim2_dyntoy/cobra.rds")
cobra <- calculate_adjp(cobra)
cobra <- calculate_performance(cobra, binary_truth="status")
cobraplot <- prepare_data_for_plot(cobra, colorscheme=cols[match(sort(unique(cobra@roc$method)),names(cols))])
plot_roc(cobraplot)
